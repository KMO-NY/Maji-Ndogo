...
-- We should send our drilling equipment to Sokoto
-- -- first, so people can drink safe filtered water from a well.
-- -- The majority of water from Amanzi comes from taps, but half of these 
-- home taps don't work because the infrastructure is broken. We need to
-- -- send out engineering teams to look at the infrastructure in Amanzi first. 
-- Fixing a large pump, treatment plant or reservoir means that
-- -- thousands of people will have running water. This means they will also not have to 
-- queue for water, so we improve two things at once
...
-- Create a table where our teams have the information they need to fix, 
-- upgrade and repair water sources
